...
def student_ranking(student_scores, student_names):
    ...
    for i in range(1, len(student_scores)+1):
        student_rank = f"{i}. {student_names[i-1]}: {student_scores[i-1]}"
        list.append(student_rank)
    return list
...
def perfect_score(student_info):
    ...
    for i in range(len(student_info)):
        if student_info[i][1] == 100:
            perfect_score_list.append(student_info[i])
    if len(perfect_score_list) > 0:
        return perfect_score_list[0]
    return []